...
function GovernorAgent (options) {
    ...
    this.log = options.log.child({agentId: this.id});
    ...
    this.net.connect().then(function () {
        ...
        return Object.keys(self.jobs);
    }).each(function (jobKey) {
        var job = self.jobs[jobKey];

        return job.registerJob().then(function () {
            job.setupSource();
        });
    }).catch(function (err) {
        self.log.error({err: err}, 'Shutting down from agent module');
...
internals.generateAgentId = function (ipAddress) {
    return ipAddress + '-' + crypto.randomBytes(3).toString('hex');
};
